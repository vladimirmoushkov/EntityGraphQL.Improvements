...
namespace EntityGraphQL.Compiler.EntityQuery
{
    internal sealed class EntityQueryNodeVisitor : EntityQLBaseVisitor<Expression>
    {
        ...
        {
            // not processing null types here
            if ((right as ConstantExpression)?.Value == null)
            {
                return null;
            }

            foreach (var supportedTypeConversion in SupportedTypesConversionsAccessor.SuportedTypesConversions)
            {
                if (right.Type == supportedTypeConversion.Type) continue;
                if (left.Type != supportedTypeConversion.Type) continue;
                var method = supportedTypeConversion.ConversionClass.GetMethod(supportedTypeConversion.ConversionMethod)!;
                if (method.IsGenericMethod)
                {
                    method = method.MakeGenericMethod(supportedTypeConversion.Type);
                }

                right = Expression.Call(method, Expression.Call(right, typeof(object).GetMethod(nameof(object.ToString))!));
                return Expression.MakeBinary(op, left, right);
            }
        ...
        private static bool IsComparissionOperator(ExpressionType expressionType)
        {
